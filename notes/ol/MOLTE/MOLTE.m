...
for problemi=1:numPP
    ...
    bm=char(text(problemi+1,4));
    if length(char(text(problemi+1,5)))==7
        offline=1;
    else
        offline=0;
    end
    NR=alldata{problemi+1, 3}; % ratio of number of alternatives
    Prior=alldata{problemi+1, 2}; % how can we get the prior
    problemClass=char(text(problemi+1,1));
    policies=alldata(problemi+1,7:6+alldata{problemi+1,6});
    tuned=zeros(1,alldata{problemi+1,6}); %record whether each policy need to be tuned
    paraProvided=zeros(1,alldata{problemi+1,6});
    alpha=zeros(1,alldata{problemi+1,6}); % store the value of tunable parameters for each policy
    for jjj=1:alldata{problemi+1,6} % find out the policies name and whether they need to be tuned
    ...
    if length(find( problemClass =='('))~=0 % this class has parameters 
   ...
   [mu_0, covM, beta_W] = priorTruthGen(problemClass, bm, paras, Prior); 
    ...
    for i=1: length(policies)
        if tuned(i)==1   % need to tune the parameter
            if offline==1 % offline objective
                alpha(i)=offtune(char(policies(i)), problemClass, M, N, bm,  paras, Prior);
            else   % online objective
                alpha(i)=ontune(char(policies(i)), problemClass, M, N, bm, paras, Prior);
            end
...
%    parfor i=1:numP
     for i=1:numP       
        ...
        for j=1:numTruth 
               ...
               [mu,beta_W]=truthGen(problemClass, mu_0, covM, beta_W);     
               mu_ALL(j,:)=mu;
        ...
        %offline OC:
        offline_avgcost(i,:)=sum(o_cost_off)/numTruth;
        
        %online average expected total rewards
        online_avgesum(i,:)=sum(ereward_sum)/numTruth/N;
        if i==1
